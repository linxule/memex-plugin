...
- ask_memex: Deep retrieval with full-content and observations
...
async def handle_list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "since": {
                        "type": "string",
                        "description": "Recency filter: '7d', '2w', '3m', or natural dates like 'yesterday', 'last week', 'march 15'"
                    },
                    "limit": {
                        "type": "integer",
                        "default": 10,
        ...
        Tool(
            name="ask_memex",
            description=(
                "Deep search the memex vault with multi-query expansion. "
                "Returns full content and matching observations."
            ),
            inputSchema={
                "type": "object",
                "properties": {
        ...
        Tool(
            name="memex_timeline",
            description=(
                "Get chronological timeline of sessions and memos for a date range. "
                "Use for 'what did I do yesterday?', 'last week's work', 'show me recent sessions'. "
                "Bypasses the search index — scans filesystem directly, catches unindexed sessions."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "date_expression": {
                        "type": "string",
                        "description": "Natural-language date: 'yesterday', 'last week', '3 days ago', 'this week', 'march 15', '2026-03-15'"
                    },
                    "project": {
                        "type": "string",
...
async def handle_call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    try:
        if name == "memex_search":
            ...
            return await handle_ask_memex(arguments)
        ...
        elif name == "memex_timeline":
            return await handle_timeline(arguments)
        else:
...
async def handle_ask_memex(args: dict) -> list[TextContent]:
    """Handle ask_memex tool."""
    memex = get_memex()
    ...
    return [TextContent(type="text", text=json.dumps(response_to_dict(response), indent=2))]
...
async def handle_graph(args: dict) -> list[TextContent]:
    ...
    if action == "backlinks":
        ...
        tasks = get_open_tasks(db_path, project=project, days=days)
        output = {"action": "tasks", "project": project, "days": days, "tasks": tasks}

    elif action == "broken":
        project = args.get("project")
        broken = get_broken_links(db_path, project=project)
        ...
        recent = get_recent_activity(db_path, days=days, project=args.get("project"))
...
async def handle_timeline(args: dict) -> list[TextContent]:
    """Handle memex_timeline tool."""
    from date_utils import parse_temporal_expression
    from temporal_scan import scan_temporal

    memex = get_memex()
    date_expr = args["date_expression"]

    parsed = parse_temporal_expression(date_expr)
    if parsed is None:
        return [TextContent(
            type="text",
            text=f"Could not parse date expression: '{date_expr}'. "
                 f"Try: yesterday, last week, 3 days ago, 2026-03-15"
        )]
        ...
        await server.run(
            ...
            InitializationOptions(
                ...
                instructions=(
                    ...
                    "Use ask_memex for complex why/how/pattern questions. "
                    "Use memex_timeline for date-based browsing ('yesterday', 'last week'). "
