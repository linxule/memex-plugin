...
- **Any code that mutates a doc's observations must `write_sidecar` for every affected doc_path** (v0.17.0, `.obs.jsonl` sidecar spec) — the vault sidecar is truth; the index is a cache, and a mutation that skips the write-through leaves the two silently diverged until the next `memex index rebuild`/`ingest-sidecars`. `extract.store_observations` takes a REQUIRED keyword-only `vault: Path | None` for exactly this reason — pass `None` only in tests or index-only tools with no vault to write into, never by omission. See `src/memex/sidecars.py` and `docs/2026-09-13-obs-sidecar-spec.md`
