...
- **Use a quoted heredoc for `--stdin` JSON** — `memex backfill obs --stdin` accepts pipes and file redirects. A quoted heredoc preserves apostrophes and backslash escapes; `echo 'JSON'` does not. If valid JSON is already in a variable, use `printf '%s\n' "$json"`. `--store-json /absolute/path/observations.json` also works wherever the file is accessible; stdin avoids additional file writes in restricted sandboxes.
...
- **Version must be bumped in THREE files together** — `pyproject.toml`, `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`; then run `uv lock` to refresh the package metadata in the tracked lockfile, and `uv run python scripts/portable_plugin.py` to carry the version into the generated Codex (`plugins/memex/.codex-plugin/plugin.json`) and Kimi Code (`kimi.plugin.json`) manifests — `tests/test_portable_plugin.py` fails if any of the five disagree. Any edit under `skills/` also needs that regeneration (same test). `src/memex/__init__.py` auto-tracks via `_read_version()`: source checkouts and plugin caches read the live pyproject.toml; installed wheels use distribution metadata. `package = true` supports both layouts. Missing any one of the three causes drift — plugin cache won't invalidate (if plugin.json is stale) or runtime version reports wrong number (if pyproject.toml is stale). **Both clones ship the `_read_version()` pattern as of 2026-05-25**; if you fork or add a new clone, port `_read_version()` immediately — DO NOT hardcode `__version__` even temporarily. The "hardcoded then forget to bump" failure mode recurred from v0.12.2 → v0.13.0 on the public clone before the pattern was ported there; treat hardcoded `__version__` as a regression on either clone.
