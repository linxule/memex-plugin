...
**Matryoshka truncation — `index_dimensions` (v0.15.0).** `dimensions` is the *native* model output (3072): what the API returns and what `embedding_cache` stores at full fidelity. `index_dimensions` is the dimension actually stored in the vec0 tables and used for KNN queries. When set below `dimensions`, embeddings are truncated to the first N dims and L2-renormalized (Gemini Embedding 2 is MRL-trained, so this is valid — ~0.26% retrieval-quality loss at 768d for a 4× smaller vector index). Omit it (or set it equal to `dimensions`) for no truncation — the default, fully backward-compatible. Because the cache keeps full 3072d, the choice is reversible: re-run the migration at any dimension, no re-embedding. `embed_query` returns the model's **native-dim** vector (v0.15.2); each query site then truncates it to *its own table's* stored dimension via `match_query_dim` right before the MATCH. (Pre-v0.15.2 `embed_query` pre-truncated to the configured `index_dimensions`, which silently degraded vector search to FTS-only during the window after `index_dimensions` was lowered in config but before `migrate-vec` ran — a native-dim query that aligns per-table matches across that window instead.)

To change an existing index's dimension: set `index_dimensions`, then run `memex index migrate-vec` (truncates + adds vec0 metadata columns in place — no API calls). Do NOT run `memex index rebuild` first — the schema's dimension-mismatch guard prints a warning and skips the destructive auto-drop precisely so a truncation never triggers a re-embed. After `migrate-vec` the dropped (larger-dim) vec pages leave the `.sqlite` file unshrunk — run `memex index vacuum` to reclaim the space. Vacuum runs `VACUUM` + `PRAGMA wal_checkpoint(TRUNCATE)` (the DB is persistent-WAL, so VACUUM stages into the `-wal` sidecar and the main file only shrinks after a TRUNCATE checkpoint).
...
- **Batch embedding MUST pass one `types.Content` per text, NOT a list of bare strings (v0.15.1)** — `client.models.embed_content(contents=[str, str, ...])` is interpreted by the google-genai SDK (1.x and 2.x) as the *parts of a single Content*, so the API returns **one** embedding for the whole list and the rest land as `None` — silent batch under-population (an N-chunk sub-batch yields 1 vector + N-1 gaps). `_aembed_one_batch` wraps each text as `types.Content(parts=[types.Part(text=t)])` so the API returns one embedding per text. An older SDK auto-wrapped bare strings as separate contents, which is why pre-v0.15.1 batch embedding worked and then silently regressed (the live index was built under the batching SDK; the gap only surfaced on new embeds). Regression: `tests/test_embedding_batch_contents.py` (a fake client asserts the `contents` arg is `Content` objects). Do NOT revert to bare strings. Symptom: `memex index embed-missing` reports `embedded: 1` per call with the rest failing.
...
- **Never JOIN `fts_content` INSIDE a sqlite-vec KNN query (v0.15.2)** — `fts_content` is FTS5 with no `path` index, so SQLite's planner picks it as the OUTER loop and re-runs the vec MATCH once per doc — O(docs × KNN), which pegs a core and hangs for minutes on a real-size index (≈3.4K docs × 178K-row KNN). Run the KNN FIRST (`vec_chunks → chunks` by rowid only; push project/type/date filters into the KNN via the metadata columns), then enrich `title/project/date` with a single batched `SELECT ... WHERE path IN (...)`. Both `hybrid_search.vector_search` and `ask._vector_candidates` follow this. The old `ask._vector_candidates` had the fatal join and silently hung `memex ask --depth thorough` (it was never exercised in verification — only `memex search` was); fixed v0.15.2 (1.39s vs unbounded). Diagnose with `EXPLAIN QUERY PLAN`: if `SCAN ... fts_content` appears as the first/outer row of a vec query, the KNN is being re-run per doc.
