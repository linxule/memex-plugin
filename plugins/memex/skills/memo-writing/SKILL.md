...
> Portable copy of `skills/memo-writing/SKILL.md` for Codex and Kimi Code, generated by `scripts/portable_plugin.py` (do not edit here). Needs the `memex` CLI on PATH. Claude Code names (the Task/Agent tool, `/memex:save`, hooks) map to your host's equivalents.
...
**Date:** (run: `date +%Y-%m-%d`)
**Project:** (run: `basename $(git remote get-url origin 2>/dev/null | sed 's/\.git$//' | xargs basename 2>/dev/null) 2>/dev/null || basename $(pwd)`)
**Topics available for [[wikilinks]]:** (run: `ls $(memex path 2>/dev/null)/topics/*.md 2>/dev/null | xargs -I{} basename {} .md | sort | tr '\n' ' ' || echo "(no topics found)"`)
...
date: [ISO date]
session_id: [full session UUID]   # $CLAUDE_CODE_SESSION_ID in-session; the transcript's id for a retry subagent
topics:
...
- `[[projects/myproject/memos/2026-03-11-memo-name|label]]` — link to a specific memo by its **full path, date prefix included**. A bare `[[memo-name]]` never resolves: memo files are named `<date>-<slug>.md`, so the slug alone is a ghost link that also inflates the crystallization report (2026-09-23: 43 such links / 64 refs, one of them OVERDUE; `memex check` now lists them separately as memo references).
