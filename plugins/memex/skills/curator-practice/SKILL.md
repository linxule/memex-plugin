...
> Portable copy of `skills/curator-practice/SKILL.md` for Codex and Kimi Code, generated by `scripts/portable_plugin.py` (do not edit here). Needs the `memex` CLI on PATH. Claude Code names (the Task/Agent tool, `/memex:save`, hooks) map to your host's equivalents.
...
- Where the backlog actually is (the four checks) — the dashboard can be weeks stale; the checks can't
...
0. Folder drift from `memex check --folders` for folders created since the last pass (sdk-cli fleet workers from `/tmp` cwds, subfolders of a repo that didn't exist yet). Investigate per the project-consolidation skill, consolidate, pin the cwd in `project_mappings` — everything downstream (condensation, signals, obs) reads the folder layout
