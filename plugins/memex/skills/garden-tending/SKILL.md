...
> Portable copy of `skills/garden-tending/SKILL.md` for Codex and Kimi Code, generated by `scripts/portable_plugin.py` (do not edit here). Needs the `memex` CLI on PATH. Claude Code names (the Task/Agent tool, `/memex:save`, hooks) map to your host's equivalents.

# Knowledge Garden Tending

## Current State

**Index:** (run: `sqlite3 "$(memex path --index 2>/dev/null)" "SELECT (SELECT COUNT(*) FROM fts_content) || ' docs, ' || (SELECT COUNT(*) FROM chunks) || ' chunks indexed'" 2>/dev/null || echo "(index unavailable)"`)

**Condensation backlog** (projects whose overview lags their memos — dated after `condensed:`, or more memos than `memos_digested:`):
(run: `memex check --condense 2>/dev/null | head -30 || echo "(memex CLI unavailable)"`)

**Topic signal backlog** (open `## Recent signals`; closed sections and archived/redirect topics excluded):
(run: `memex check --signals 2>/dev/null | head -16 || echo "(memex CLI unavailable)"`)
<!-- NOTE: `!`command`` injections get slash-command argument substitution ($1, $2, $ARGUMENTS) applied BEFORE execution, so any `$<digit>` in them is unsafe — keep shell logic in the CLI, not inline here. -->

**Existing topics (use [[?name]] for anything NOT in this list):**
(run: `ls $(memex path 2>/dev/null)/topics/*.md 2>/dev/null | xargs -I{} basename {} .md | sort | tr '\n' ' ' || echo "(no topics found)"`)
...
`--condense` parses both memo filename formats (`YYYY-MM-DD-slug` and legacy `YYYYMMDD-HHMM-slug`) — a hand-rolled string compare sorts every legacy name as "newer" (2026-09-23: ~350 false "new" memos vs ~60 real). A substantial overview with no `condensed:`/`memos_digested:` shows as **unstamped**: stamp it, don't re-condense from scratch. GAP catches same-day memos and memos moved in by a consolidation.
...
**Interpretation:**
- **5+ NEW memos** (`memex check --condense`) → needs condensation
- **`unstamped` (substantial overview, no `condensed:`/`memos_digested:`)** → NOT never-condensed; the overview is current but predates the frontmatter convention (or was hand-written). Just add `condensed:`/`memos_digested:` frontmatter — don't re-condense from scratch. Distinguishing this from a true stub avoids wasted re-condensation work.
...
```bash
memex check --signals          # table: open count, oldest signal, topic updated:, trail flag
memex check --signals --json   # per-topic `open`, `since_updated`, `oldest`/`newest`, `lines`
```
...
`--signals` already excludes `## Recent signals (closed — …)` sections, `> **Closed …` blockquote sections, and archived/redirect topics — the hand-rolled grep counted all of them (2026-08-05, 08-25, 09-08 and 09-23 each re-learned that). It still counts duplicates and signals whose content was integrated without clearing the bullet, so read before folding; never trust the count alone.
...
**Wave coordination** (fold + condense agents in parallel — 2026-09-23 ran 9 fold + 6 condense agents with zero write conflicts):
